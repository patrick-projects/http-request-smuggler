...
//        }
    ...
    private List<IScanIssue> reflectScan(byte[] original, IHttpService service) {
        ...
        // ISO-8859-1 mirrors Burp's helpers.bytesToString used a few lines above and
        // preserves raw HTTP bytes verbatim through the String round-trip.
        Resp bad = request(service, TRIGGER.getBytes(StandardCharsets.ISO_8859_1), 0, true);
        if (Utilities.contains(bad, "Incapsula incident ID")) {
            return null;
        }

        TurboHelper helper = new TurboHelper(service, true);
        // ISO-8859-1 preserves raw HTTP bytes verbatim; using the platform default
        // charset corrupts high-bit bytes from smuggling payloads on UTF-8 JVMs.
        helper.queue(new String(original, StandardCharsets.ISO_8859_1));
        helper.queue(TRIGGER);
        List<Resp> results = helper.waitFor();
        if (results.size() < 2) {
            return null;
        ...
        byte[] firstResponse = results.get(0).getReq().getResponse();
        if (firstResponse == null
                || "null".equals(new String(firstResponse, StandardCharsets.ISO_8859_1))) {
            return null;
        }
