...
public abstract class SmuggleScanBox extends Scan {
    ...
    SmuggleScanBox(String name) {
        ...
        // Comma-separated list of host substrings that the "ignore probable FPs" toggle skips
        // for HTTP/2 pseudo-header probes. Historically ".yahoo,.aol" were hardcoded with a
        // latent operator-precedence bug that silently dropped ".aol" hosts regardless of the
        // toggle; this list now governs that filter and is reachable from the settings UI.
        DesyncBox.sharedSettings.register("h2 fp host substrings", ".yahoo,.aol");
    ...
    /**
     * Response markers that strongly indicate the request was answered by a major
     * CDN/WAF layer rather than the origin. Used by {@link #suspectedFalsePositive}
     * for permutations that are known to FP when the front-edge normalizes weird
     * HTTP/2 header bytes before the request reaches the back-end.
     */
    private static final String[] KNOWN_CDN_RESPONSE_MARKERS = {
            "X-Amz-Cf-",            // Amazon CloudFront
            "cf-ray",                // Cloudflare
            "x-fastly-request-id",   // Fastly
            "AkamaiGHost"            // Akamai (typically in the Server header)
    };
    ...
    boolean suspectedFalsePositive(String permutation, Resp response) {
        ...
        switch (permutation) {
            case "h2space":
                // The h2space technique relies on a raw space inside an HTTP/2 header
                // name. Every major CDN aggressively normalizes this on ingress, so a
                // positive scan against a CDN-fronted host is overwhelmingly likely to
                // be detection of the normalization layer rather than a real back-end
                // desync. The filter intentionally only applies to this permutation so
                // that techniques with different physics (e.g. content-encoding,
                // chunked-extension) still report on CDN-fronted hosts.
                for (String marker : KNOWN_CDN_RESPONSE_MARKERS) {
                    if (Utilities.contains(response, marker)) {
                        return true;
                    }
                }
                return false;
    ...
    Resp leftAlive(byte[] req, IHttpService service) {
        ...
        byte[] keepalive = Utilities.setHeader(req, "Connection", "keep-alive", true);
    ...
    static Resp buildPoc(byte[] req, IHttpService service, HashMap<String, Boolean> config) {
        try {
            byte[] badMethodIfChunked = Utilities.setHeader(req, "Connection", "keep-alive", true);
