...
public class ImplicitZeroScan extends SmuggleScanBox {
    ...
    /** Single-connection attack retries per technique during Smuggle Targets / passive scan. */
    static final int SCAN_ATTEMPTS_PER_TECHNIQUE = 9;

    HashMap<String, Mapping> recordedGadget = new HashMap<>();
    HashSet<String> reportedStatus = new HashSet<>();

    // Set on a confirmed CL.0 so callers (e.g. the desync agent) can replicate the exact
    // request that worked instead of a synthetic PoC. Purely additive; does not affect scanning.
    byte[] lastConfirmedAttack;
    byte[] lastConfirmedBase;
    ...
    public boolean doConfiguredScan(byte[] baseReq, IHttpService service, HashMap<String, Boolean> config) {
        return doConfiguredScan(baseReq, service, config, null);
    }

    /**
     * @param preferredGadgetLine when non-null (e.g. from the desync agent UI), use this smuggled
     *                            request line instead of {@link #selectGadget}'s auto-pick.
     */
    public boolean doConfiguredScan(byte[] baseReq, IHttpService service, HashMap<String, Boolean> config,
                                    String preferredGadgetLine) {
        ...
        Mapping gadget;
        if (preferredGadgetLine != null && !preferredGadgetLine.trim().isEmpty()) {
            gadget = mappingForRequestLine(preferredGadgetLine.trim());
        } else {
            gadget = selectGadget(service, req, baseReq);
            if (gadget == null) {
                gadget = new Mapping("TRACE / HTTP/1.1", "405 Method Not Allowed", true);
            }
        }

        int attempts = SCAN_ATTEMPTS_PER_TECHNIQUE;
        short status = 0;
        ...
        for (int i=0; i<attempts; i++) {
            ...
            if (gadget.worked(resp)) {
                ...
                lastConfirmedAttack = attack;
                lastConfirmedBase = baseReq;
    ...
    /** Maps a smuggled request line to response markers (same heuristics as {@link #selectGadget}). */
    static Mapping mappingForRequestLine(String requestLine) {
        String lower = requestLine.toLowerCase();
        if (lower.contains("favicon")) {
            return new Mapping(requestLine, "Content-Type: image/", true);
        }
        if (lower.contains("robots.txt")) {
            return new Mapping(requestLine, "llow:", false);
        }
        if (lower.contains("wrtztrw")) {
            return new Mapping(requestLine, "wrtztrw", false);
        }
        if (lower.contains("sitemap")) {
            return new Mapping(requestLine, "Content-Type: application/xml", true);
        }
        if (lower.startsWith("trace ")) {
            return new Mapping(requestLine, "405 Method Not Allowed", true);
        }
        if (lower.contains("http/2.2")) {
            return new Mapping(requestLine, "505 HTTP", true);
        }
        int sp1 = requestLine.indexOf(' ');
        int sp2 = sp1 >= 0 ? requestLine.indexOf(' ', sp1 + 1) : -1;
        String path = sp2 > sp1 ? requestLine.substring(sp1 + 1, sp2) : requestLine;
        String lookFor = path.startsWith("/") && path.length() > 1 ? path.substring(1) : path;
        return new Mapping(requestLine, lookFor, false);
    }
