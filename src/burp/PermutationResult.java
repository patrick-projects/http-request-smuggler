...
//            }
    ...
    MontoyaRequestResponse consistent(int iterations) {
        ...
        for (int i = 0; i < iterations; i++) {
            MontoyaRequestResponse surprise = retryIfTransient(
                    () -> Scan.request(permutor.transformIntoBaseline(base, canaryHeader, false), true),
                    canaryPresent.status());
            if (surprise != null) {
                return surprise;
            }
        }
        ...
        for (int i = 0; i < iterations; i++) {
            MontoyaRequestResponse surprise = retryIfTransient(
                    () -> Scan.request(permutor.transformIntoBaseline(base, canaryHeader, true), true),
                    canaryMissing.status());
            if (surprise != null) {
                return surprise;
            }
        }
        ...
        for (int i = 0; i < iterations; i++) {
            MontoyaRequestResponse surprise = retryIfTransient(
                    () -> Scan.request(permutor.transformIntoHidden(base, canaryHeader, false), true),
                    hiddenCanaryPresent.status());
            if (surprise != null) {
                return surprise;
            }
        }
        ...
        for (int i = 0; i < iterations; i++) {
            MontoyaRequestResponse surprise = retryIfTransient(
                    () -> Scan.request(permutor.transformIntoHidden(base, canaryHeader, true), true),
                    hiddenCanaryMissing.status());
            if (surprise != null) {
                return surprise;
            }
        }
    ...
    /**
     * Runs {@code probe} once and compares against {@code baselineStatus}. If the
     * mismatching status looks like a transient network/rate-limit blip (status 0
     * or 429), re-runs the probe once before declaring the host inconsistent.
     *
     * <p>Without this, a single rate-limit response or connection blip during a
     * confirmation loop can cause the entire permutation to be discarded as
     * unstable, which silently drops real findings on noisy targets. This mirrors
     * the transient-status filter already used by
     * {@link #recordInstability(MontoyaRequestResponse)}.</p>
     */
    private static MontoyaRequestResponse retryIfTransient(
            java.util.function.Supplier<MontoyaRequestResponse> probe, int baselineStatus) {
        MontoyaRequestResponse surprise = probe.get();
        int surpriseStatus = surprise.status();
        if (surpriseStatus == baselineStatus) {
            return null;
        }
        if (isTransientStatus(surpriseStatus)) {
            MontoyaRequestResponse retry = probe.get();
            int retryStatus = retry.status();
            if (retryStatus == baselineStatus || isTransientStatus(retryStatus)) {
                return null;
            }
            return retry;
        }
