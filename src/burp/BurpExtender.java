...
public class BurpExtender implements IBurpExtender, IExtensionStateListener, BurpExtension {
    ...
    public void initialize(MontoyaApi api) {
        ...
        // Register the target-picker tab outside of DAST mode (DAST already integrates
        // the scanner directly and the picker is an operator UX affordance).
        if (!Utilities.isBurpDAST()) {
            try {
                SmuggleTargetTab tab = new SmuggleTargetTab(api);
                api.userInterface().registerSuiteTab(SmuggleTargetTab.TAB_TITLE, tab.component());
            } catch (Throwable t) {
                Utilities.err("SmuggleTargetTab failed to register: " + t.getMessage());
            }
        }
    }
    ...
    public void registerExtenderCallbacks(final IBurpExtenderCallbacks callbacks) {
        ...
        if (!Utilities.isBurpDAST()) {
            ...
            callbacks.registerContextMenuFactory(new DesyncReplicatorMenu());
        }
