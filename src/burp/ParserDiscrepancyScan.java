...
//        permutors.add(new EarlyBodyPair("rsplit", HttpHeader.httpHeader("Arrr", "foo\r\rbar"), PermutationOutcome.VISIBLE));
        ...
        if (researchMode) {
            // Additional hide techniques and early-body permutors that are kept off the
            // default scan path because they can be noisier than the core five above.
            // Research mode is itself opt-in via globalSettings, so these run only when
            // the operator has accepted the noise/coverage trade-off.
            permutors.add(new HiddenPair("underflip", HideTechnique.SPACE, PermutationOutcome.NODESYNC));
            permutors.add(new HiddenPair("skiphop", HideTechnique.SKIPHOP, PermutationOutcome.NODESYNC));

            permutors.add(new EarlyBodyPair("null-term",
                    HttpHeader.httpHeader("X-Null", "foo\0\r\n{significant}"),
                    PermutationOutcome.NODESYNC));
            permutors.add(new EarlyBodyPair("invalidsplit",
                    "Bad name:\r\n\r\n{significant}",
                    PermutationOutcome.HIDDEN));
            permutors.add(new EarlyBodyPair("nsplit-name",
                    HttpHeader.httpHeader("X-Foo", "foo\n\nbar: x"),
                    PermutationOutcome.HIDDEN));
            permutors.add(new EarlyBodyPair("rsplit",
                    HttpHeader.httpHeader("Arrr", "foo\r\rbar"),
                    PermutationOutcome.VISIBLE));
        }
        ...
        int establishedBaseStatus = 0;
        int iterationsSinceLastStabilityCheck = 0;
        // Periodic mid-scan recheck cadence. Small enough to catch drift before too
        // many permutations record findings against a stale baseline, large enough
        // that the extra requests don't dominate the scan budget on small targets.
        final int stabilityRecheckInterval = 8;
        ...
        for (PermutationPair permutor: permutors ) {
            for (SignificantHeader canaryHeader : canaryHeaders) {
                try {
                    if (researchMode && testedServerStability
                            && iterationsSinceLastStabilityCheck >= stabilityRecheckInterval) {
                        // If the host has drifted since we last looked, abandon this scan
                        // rather than recording further findings against a stale baseline.
                        // Three probes are enough evidence because we already established
                        // stability earlier in this scan.
                        boolean stillStable = true;
                        for (int probe = 0; probe < 3; probe++) {
                            if (request(base, true).serverStatus() != establishedBaseStatus) {
                                stillStable = false;
                                break;
                            }
                        }
                        if (!stillStable) {
                            Utilities.log("ParserDiscrepancyScan: aborting "
                                    + original.httpService().host()
                                    + " (host became unstable mid-scan)");
                            return null;
                        }
                        iterationsSinceLastStabilityCheck = 0;
                    }
                    iterationsSinceLastStabilityCheck++;
                    ...
                    if (result.isInteresting()) {
                        if (!researchMode) {
                            ...
                            if (!testedServerStability) {
                                ...
                                establishedBaseStatus = result.baseStatus;
                                iterationsSinceLastStabilityCheck = 0;
                            }
                    ...
                    if (e.toString().contains("java.net.UnknownHostException")) {
                        // Surface DNS failures so users can tell a host was skipped instead of
                        // assuming the scan completed cleanly.
                        Utilities.err("ParserDiscrepancyScan: skipping " + original.httpService().host()
                                + " (unresolvable host)");
                        return null;
                    }
                    // Route scan exceptions through the user-visible error stream rather than
                    // dumping a stack trace to stderr where it would be invisible to operators.
                    Utilities.err("ParserDiscrepancyScan: unexpected error on "
                            + original.httpService().host() + " permutor=" + permutor.getName()
                            + " canary=" + canaryHeader.getLabel() + ": "
