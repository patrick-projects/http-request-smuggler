    ...
    static byte[] applyDesync(byte[] request, String header, String technique) {
        ...
        if (header.equals("Transfer-Encoding: ")) {
            if (technique.equals("commaCow")) {
                ...
                // ISO-8859-1 keeps each byte 1:1 so toLowerCase() only touches ASCII
                // letters and the high-bit bytes from other permutations survive.
                request = (new String(request, java.nio.charset.StandardCharsets.ISO_8859_1))
                        .toLowerCase().getBytes(java.nio.charset.StandardCharsets.ISO_8859_1);
