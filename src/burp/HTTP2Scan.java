...
public class HTTP2Scan extends SmuggleScanBox implements IScannerCheck {
    ...
    /**
     * Shared "likely-false-positive" host filter for the HTTP/2 pseudo-header
     * probes. Historically inlined in {@link HTTP2Scheme} and {@link HTTP2Method}
     * with broken operator precedence that silently skipped a class of hosts
     * regardless of the global "ignore probable FPs" toggle.
     *
     * <p>The filter is opt-in via {@code globalSettings("ignore probable FPs")}
     * and the host substrings are read from
     * {@code globalSettings("h2 fp host substrings")} (comma-separated, trimmed).
     * Defaults preserve historical behavior.</p>
     */
    static boolean shouldSkipForFP(IHttpService service) {
        if (service == null || !Utilities.globalSettings.getBoolean("ignore probable FPs")) {
            return false;
        }
        ...
        String substringsCsv;
        try {
            substringsCsv = Utilities.globalSettings.getString("h2 fp host substrings");
        } catch (Throwable t) {
            // Older bulkScan-all.jar may not know the setting; fall back to historical defaults.
            substringsCsv = ".yahoo,.aol";
        }
        if (substringsCsv == null || substringsCsv.isEmpty()) {
            return false;
        }
        for (String substring : substringsCsv.split(",")) {
            substring = substring.trim();
            if (!substring.isEmpty() && host.contains(substring)) {
                return true;
            }
        }
        return false;
...
//        }
    ...
    public static Resp h2request(IHttpService service, byte[] req, boolean transform) {
        ...
        LinkedList<Pair<String, String>> h2headers = H2Connection.Companion.buildReq(new HTTP2Request(Utilities.helpers.bytesToString(req)), transform);
